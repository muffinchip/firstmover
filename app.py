...
def gmail_oldest_for_query(credentials: Credentials, query: str):
    """Return oldest matching message timestamp (ms) for a query; safe page cap to avoid timeouts."""
    ...
    while True:
        ...
        if ids:
            last_id = ids[-1]["id"]  # oldest on this page
...
WELCOME_QUERIES = {
    # Stricter subjects to reduce false positives (password resets, promos)
...
@app.route("/")
def index():
    # starting fresh clears previous manual entries so the UI matches the form
...
def auth_google():
    ...
    # after login, go to /results which will merge any manual entries stored in session
...
def results():
    ...
    # 1) Read manual from POST, else from session (to survive Google auth redirect)
    manual = {}
    if request.method == "POST":
        ...
        for key in platform_keys:
            ...
            if m and y:
                ...
                if ms:
                    manual[key] = ms
        # persist for later (e.g., they click Google after submitting manual)
        session["manual_entries"] = manual
    else:
        manual = session.get("manual_entries", {}) or {}

    # 2) Gmail (verified), only if scope was granted
    gmail_hits = {}
    creds = get_google_credentials()
    if creds:
        for key, q in WELCOME_QUERIES.items():
            ...
            try:
                ...
                ts = None
            if ts:
                # If user provided a manual month/year, prefer Gmail ONLY if within 12 months
                gmail_hits[key] = ts

    # 3) Gmail baseline (first-ever email -> Gmail join)
    ...
    # 4) Resolve conflicts (manual vs gmail)
    resolved = {}
    keys = set(list(platform_keys))
    for key in keys:
        m_ms = manual.get(key)
        ...
        if m_ms and g_ms:
            ...
            # If diff < 12 months -> VERIFIED (use Gmail). Else -> SELF-REPORT (use manual, show email hint).
    ...
    # 5) Build cards & percentiles
    platforms = []
    percentiles_all, percentiles_verified = [], []
    ...
    for key, info in resolved.items():
        if key not in CURVES:  # skip if no curve
            continue
        add_platform_card(platforms, percentiles_all, percentiles_verified, key, info["date_ms"], verified=info["verified"], email_hint_ms=info.get("email_hint_ms"))
...
def add_platform_card(platforms, percentiles_all, percentiles_verified, key, ts, verified=False, email_hint_ms=None):
    # If TS predates launch, clamp to launch
    ld = launch_date(key)
...
def diag():
    ...
    safe = {
        ...
        "manual_entries_cached": bool(session.get("manual_entries"))
    }
    return jsonify(safe)
